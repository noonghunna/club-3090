...
| **Qwen3_5MTP lacks pipeline-parallel support** | 🔴 No upstream issue filed yet | `vllm/quad` uses PP=2 × TP=2 to keep TP all-reduce inside NVLink pairs. vLLM dev205 rejects `{"method":"mtp"}` on this shape with `Qwen3_5MTP` because the draft model does not implement `SupportsPP`. | `quad.yml` disables MTP. Use `quad-pairs.yml` for MTP n=3 on two independent TP=2 endpoints. |
