...
| **PN25 — Inductor-safe `silu_and_mul` pool** | 🔴 Shipped on Genesis v7.65 dev BUT blocked by worker-fork registration bug | Sandermage's `torch.library.custom_op` for `forward_native` (sister of our former `patch_pn12_compile_safe_custom_op.py`). Required to close [club-3090#16](https://github.com/noonghunna/club-3090/issues/16) on real IDE-agent workloads. Worker-fork registration bug filed as [genesis-vllm-patches#16](https://github.com/Sandermage/genesis-vllm-patches/issues/16) — `_register_op_once()` in-process flag resets when vLLM workers spawn, re-decoration calls `torch.library.infer_schema` during dynamo trace which fails. **Escalated to Sandermage as urgent 2026-05-01 PM** after we reproduced Cliff 1 mech B on a 6K-char IDE-agent-shape prompt. PR offered. | None — must stay opt-in OFF until upstream fix. Mitigation: route IDE-agent workloads to `tools-text.yml` (PN8 closes Cliff 1 via different mechanism that reaches the compile path). |
| **DS conv state layout × spec-decode crash** (we filed 2026-05-01 PM as [genesis-vllm-patches#17](https://github.com/Sandermage/genesis-vllm-patches/issues/17)) | 🟡 Filed; in Sandermage's PROD env-var set | `VLLM_SSM_CONV_STATE_LAYOUT=DS` (Sandermage's PROD setting) crashes the engine with `NotImplementedError: DS conv state layout does not yet support speculative decoding with mamba_cache_mode='align' (num_accepted_tokens > 1)` on certain prompt shapes (LCB v6 LeetCode-style problems triggered it on every request; canonical narrative+code prompts don't trigger it). Specific to MTP K=3 + AL>1 + certain prefill shapes. | Pending Sandermage's call. Workaround if it fires: drop `VLLM_SSM_CONV_STATE_LAYOUT=DS` from compose env (untested perf cost). |
