...
| **P104 FA max_seqlen_k runtime clamp** (we built 2026-04-30) | 🔴 Branch `club-3090-cliff1-prep` in local Genesis clone — **PR-ready, not yet pushed to Sandermage's repo** | Implements the Genesis-style FA call-site clamp from issue #11. Env-gated `GENESIS_ENABLE_FA_MAX_SEQLEN_CLAMP=1`, runtime-only (skips under cudagraph capture), never under-clamps. ~260 lines following Genesis's text-patch infrastructure. | Already shipped locally — when Sandermage merges (or when GENESIS_PIN bumps to a commit including it), we can enable in compose env. |
| **P101 anchor drift fix** (we found + fixed 2026-04-30) | 🔴 Same branch as P104 — PR-ready | P101 was silently no-op on vLLM dev205+ — anchor `_arange_cache[...]` no longer matches upstream `torch.arange(...)` form. apply_all reported "applied" misleadingly. Anyone running v7.62.x with `GENESIS_ENABLE_P101=1` on dev205+ gets a no-op. | Same branch ships the fix. |
