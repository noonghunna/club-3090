...
| **P104 FA max_seqlen_k runtime clamp** (we built 2026-04-30) | 🟡 Branch `club-3090-cliff1-prep` in local Genesis clone — **held back from PR pending Sandermage's response on #11** | Implements the Genesis-style FA call-site clamp from issue #11. Env-gated `GENESIS_ENABLE_FA_MAX_SEQLEN_CLAMP=1`, runtime-only (skips under cudagraph capture), never under-clamps. ~260 lines following Genesis's text-patch infrastructure. | Already shipped locally — when Sandermage merges (or when GENESIS_PIN bumps to a commit including it), we can enable in compose env. |
