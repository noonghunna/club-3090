...
| **PN25 — Inductor-safe `silu_and_mul` pool** | ✅ Closed in Genesis v7.65 dev tip; opt-in only on our composes | Sandermage's `torch.library.custom_op` for `forward_native` (sister of our former `patch_pn12_compile_safe_custom_op.py`). Currently SKIP'd on our composes (`GENESIS_ENABLE_PN25_SILU_INDUCTOR_SAFE=0`) — PN12 covers Cliff 1 mech B on TQ3 paths; PN25 is for the compile-mode silu_and_mul code path which doesn't currently fire on our config. | Enable if PN25 surface starts firing. Local sidecar already removed. |
