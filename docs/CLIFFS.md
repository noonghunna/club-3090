...
> **Rig-class caveat — "known good" is rig-specific, not universal.** The v7.66 / v7.69 verdicts above were measured on **bare-metal Ubuntu 2× 3090 PCIe with default Docker runtime**. Configurations that differ materially from that baseline have surfaced separate, older bugs that are NOT closed by Genesis pin choice within the v7.x cycle. Specifically: [@lexhoefsloot reports](https://github.com/noonghunna/club-3090/issues/49) `vllm serve` hits an `uvloop` event-loop crash before reaching GPU initialization on a 3× 3090 Proxmox VE / Debian 12 / kernel 6.17.2 / `default-runtime: nvidia` setup with override compose — the crash predates v7.66 and is independent of `GENESIS_ENABLE_P87`. If you're not on the bare-metal baseline and hit a pre-engine crash, please file with rig-class details (Proxmox / LXC / WSL / kernel / runtime / override-compose shape) so we can disambiguate environment effects from Genesis-pin effects.
