...
⚠️ **Streaming tool-calls.** These composes default to `--tool-call-parser qwen3_xml`. The alternative `qwen3_coder` has an **open streaming-tool-call bug** ([#145](https://github.com/noonghunna/club-3090/issues/145)) on reasoning-enabled composes — tool-calls can be silently dropped over a *streaming* connection, and Copilot streams — so we ship `qwen3_xml` instead (the upstream streaming-parser rewrite, [vLLM #45413](https://github.com/vllm-project/vllm/pull/45413), will revisit both parsers). If you've manually pinned `qwen3_coder` and streaming tool-calls vanish, switch the compose's `command:` block back to `qwen3_xml` and relaunch. Non-streaming clients are unaffected either way.
