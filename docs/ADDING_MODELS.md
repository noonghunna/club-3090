...
> 3. **Catalog it on YOUR rig — the LOCAL layer (no PR, nothing tracked).** `python3 scripts/lib/profiles/promote.py --spec-file <spec>` (`--layer local` is the default) writes the gitignored `scripts/lib/profiles-local/`, giving you `launch.sh`/`switch.sh` discovery, the VRAM projection and guard coverage **without** touching a tracked file — `git pull` can never clobber it. Ready to contribute it upstream later? `export_pr.py` turns it into a PR bundle. Walked end-to-end in [BRING_YOUR_OWN.md](BRING_YOUR_OWN.md) §4–§5. Removing it again is `python3 scripts/lib/profiles/demote.py --slug local/<name>` — it unregisters the slug and deletes the files it wrote, so a local model is never a one-way door.
