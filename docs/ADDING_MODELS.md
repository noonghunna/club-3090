...
End-to-end workflow for onboarding a new model into club-3090's **central registry** — `scripts/lib/profiles/registry.yaml` (the single source of truth; `compose_registry.py` is its loader + API surface) plus the profile / engine / drafter / calibration YAMLs it points at. Registering a model here is what makes it a first-class catalog citizen: **`launch.sh` and `switch.sh` both resolve it by slug** (both are registry-derived — you never edit the launchers), the VRAM/KV projection (`kv-calc`) knows it, and the guard tests cover it. Pairs with [KV_MATH.md](KV_MATH.md) (math reference) and [ARCHITECTURE.md](ARCHITECTURE.md) (current stack state).

> **Adding a model? Four paths — pick the lightest that fits:**
>
...
> 3. **Catalog it on YOUR rig — the LOCAL layer (no PR, nothing tracked).** `bash scripts/catalog.sh register --compose <your-compose>.yml` reads the compose you already have and fills in what it can, then prints every value it resolved before writing. Slugs take the same `<engine>/<name>` shape as curated ones (provenance is the entry's `origin` field, not the name), so **an engine we do not ship is a first-class case**: pass `--engine my-build --engine-type llama.cpp` and a local engine profile is written from evidence. `switch.sh --local` lists what you registered. The hand-authored `promote.py --spec-file` path still works and is what `register` builds underneath. Walked end-to-end in [BRING_YOUR_OWN.md](BRING_YOUR_OWN.md).
> 4. **Promote a model into the curated catalog — i.e. register it in the central `registry.yaml`** — *this page*. The heavier task: validated composes, a registry entry per compose (the SoT that `launch.sh`/`switch.sh` derive from), profile-compat coverage, calibration anchors, real benchmarks, per-model gotchas. The high-confidence backbone — **not** a prerequisite for serving.
>
> Paths 1–3 (serve + **tune** + **validate** + **catalog** your own model without touching core files) are walked end-to-end in [BRING_YOUR_OWN.md](BRING_YOUR_OWN.md) — start there if you're not yet cataloging. This page is the promotion step *after* you've validated a config there.
...
| Gateway + URLs | `litellm-emit.sh` derives `services/litellm/config.yaml` routes from the registry's **`gateway=True`** entries (#1078) — no more hand-written `:<port>` route stanzas to forget (#1062). Extra served-names ride `serve_aliases`. **Your rig serves a different scene of that model** (e.g. `dual-fast` instead of the curated `dual-max`)? Pin it (`switch.sh --set-default <slug>`), run `litellm-emit.sh --local` → gitignored `services/litellm/config.local.yaml` routed to your pinned scene, and set `LITELLM_CONFIG=./config.local.yaml` in `services/litellm/.env` — never hand-edit the tracked config (#1325). | set `gateway=True` on the ONE slug LAN clients should hit | `test-litellm-ports-resolve` · `test-default-url-gateway` · `test-litellm-local-pin` |
...
| PR export | Validated LOCAL model → ready-to-commit CORE bundle via `export_pr.py`: emits `models/<id>.yml`, the core-layout compose, and a `registry-entry.yaml` entry file into `--out` (the `entries:` map in exactly the registry.yaml data subset — **not** a `compose_registry.py` source patch). | run it after LOCAL validation | `--check` mode; `test_export_pr.py` |
...
| `test-litellm-generate` · `test-litellm-ports-resolve` · `test-litellm-local-pin` | gateway block ≡ registry derivation; ports resolve; `--local` honours the rig's scene pin without touching tracked files |
