...
try {
    ...
    if ($gpuLines) {
        $gpuCount = ($gpuLines -split "`n" | Where-Object { $_ -match 'GPU' }).Count
    }
} catch { }
        ...
        if ($gpuCount -gt 2) {
            try {
                ...
                if ($topo -match '\bNV[0-9]+\b') {
                    ...
                    if ($partial) {
                        $_NvlinkPartial = 1
                        Log "$gpuCount GPUs - NVLink found but mesh NOT fully connected (pairwise bridges)"
                    } else {
                        Log "$gpuCount GPUs - NVLink full mesh, enabling NVLink mode"
                    }
                } elseif (Test-PcieP2pAvailable) {
                    $_NvlinkEnabled = 1
                    $_P2PLevel = if ($env:NCCL_P2P_LEVEL) { $env:NCCL_P2P_LEVEL } else { "PHB" }
                    Log "$gpuCount GPUs - no NVLink, but P2P=OK - auto-enabling PCIe P2P"
                } else {
                    $_NvlinkEnabled = 0
                    Log "$gpuCount GPUs - no NVLink, no P2P - using PCIe mode"
                }
            } catch {
                $_NvlinkEnabled = 0
            }
        } elseif ($gpuCount -eq 2) {
            try {
                $topo = nvidia-smi topo -m 2>$null
            ...
            $_NvlinkEnabled = 0
            Log "$gpuCount GPU(s) - skipping NVLink detection"
        }
    }
    default {
        Write-Error "invalid NVLINK_MODE=$NvlinkMode (must be auto|force_on|force_off|pcie_p2p)"
        exit 1
    }
}
...
Write-Host "_NVLINK_ENABLED=$_NvlinkEnabled"
