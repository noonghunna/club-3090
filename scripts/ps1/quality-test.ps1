...
# Reachability probe
try {
    $probe = Invoke-WebRequest -Uri "$url/v1/models" -TimeoutSec 8 -ErrorAction SilentlyContinue
    if (-not $probe) {
    ...
    if ($ApiKey) {
        try {
            ...
            $probe = Invoke-WebRequest -Uri "$url/v1/models" -Headers $headers -TimeoutSec 8 -ErrorAction SilentlyContinue
        } catch { }
    }
    if (-not $probe) {
...
if ($modelExplicit) { $cmdArgs += "--model"; $cmdArgs += $model }
...
Log "Running: benchlocal-cli $($cmdArgs -join ' ')"

# Execute benchlocal-cli
benchlocal-cli @cmdArgs 2>&1
exit $LASTEXITCODE
