...
#   bash scripts/report.sh --agentic         # adds bench-agentic.sh output (1 session × 12 turns, ~8 min)
#   bash scripts/report.sh --full            # ALL five: verify + stress + soak + bench + agentic (~43 min, the canonical "everything" pass for cross-rig contributions)
...
# ---------------------------------------------------------------------------
# Optional: agentic bench (multi-turn prefill stress)
# ---------------------------------------------------------------------------

if [[ $DO_AGENTIC -eq 1 ]]; then
  section "bench-agentic.sh output"
  if [[ -f scripts/bench-agentic.sh ]]; then
    SESSIONS=1 bash scripts/bench-agentic.sh 2>&1 | redact | details "agentic bench (1 session × 12 turns, multi-turn prefill stress, ~8 min)"
  else
...
_Generated by `bash scripts/report.sh`. Flags: `--verify` (verify-full), `--stress` (verify-stress 7/7 incl. Cliff 2 needles), `--soak` (SOAK_MODE=continuous, catches Cliff 2b), `--bench` (canonical TPS), `--agentic` (multi-turn prefill stress, ~8 min), `--full` (all five, ~43 min). Use `--no-redact` to disable redaction (internal sharing only)._
EOF
