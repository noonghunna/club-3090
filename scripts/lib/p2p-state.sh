...
p2p_opportunity_hint() {
  ...
  if p2p_reports_cns; then
    echo "ℹ interconnect: ${count} GPUs on a P2P-capable layout (same root complex) with peer access OFF — \`topo -p2p\` reports CNS, which is the stock driver refusing P2P on GeForce cards rather than a hardware limit. A patched kernel module can unlock it."
      ...
      echo "  ⚠️ ENGINE MATTERS. Those gains are vLLM tensor-parallel numbers. On llama.cpp / ik-llama \`-sm layer\` the measured gain is +0.2% decode and ~+1% prefill (disc #921) — layer split forwards activations with plain copies, so there is no all-reduce for a peer path to accelerate. If you serve only GGUF, this lever buys you NOTHING and is not worth a DKMS rebuild."
