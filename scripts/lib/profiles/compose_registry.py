...
def _entry(
    *,
    ...
    # Opt THIS SLUG out of the hardware mem_util_safe ceiling (see
    # launch_compat._mem_util_env). The ceiling is a card-wide safety knob; a
    # slug that has MEASURED its own peak headroom sets this instead of
    # loosening the ceiling for every other model sharing the card.
    # Use ONLY with a measured peak-under-load figure in the status_note.
    mem_util_unclamped=False,
    status="production",
    status_note=None,
    category=None,
    ...
    entry = {
        ...
        "mem_util": mem_util,
        "mem_util_unclamped": mem_util_unclamped,
...
COMPOSE_REGISTRY = {
    ...
    # Qwen3.8-27B AWQ W4A16 -- the SPEED tier next to the official-FP8 accuracy tier.
    # Separate v0.27.1 engine channel because its MTP evidence is v0.27.1-specific
    # (see the note). Not a DEFAULTS target.
    "vllm/qwen38-27b-dual-awq": _entry(
        model="qwen3.8-27b", weights_variant="awq-w4a16", workload="long-ctx-single", chat_template="native",
        engine="vllm-stable-v0271", drafter="qwen-mtp-builtin", kv_format="fp8_e4m3",
        tp=2, max_ctx=262144, max_num_seqs=2, mem_util=0.95, mem_util_unclamped=True,
        compose_path="models/qwen3.8-27b/vllm/compose/dual/awq-w4a16/fp8.yml",
        default_port=8094,
        kvcalc_key="SKIP",
        status="experimental",
        status_note="Qwen3.8-27B AWQ W4A16_ASYM g128 (barrydeen/Qwen3.8-27B-AWQ-4bit, compressed-tensors/pack-quantized), 2-card TP=2 @262K + fp8_e4m3 KV + prefix caching + MTP n=3, on the vllm-stable-v0271 channel. THE SPEED TIER, complementing vllm/qwen38-27b-dual-max (official FP8, the accuracy tier). FIRST MEASURED TPS FOR THIS MODEL ON THIS STACK (2026-08-15, 2x3090 = 3090 Ti + 3090 sm_86, PCIe x8, P2P GNS, caps 250/270 W, vLLM v0.27.1): decode 80.24 narrative / 102.42 code TPS with MTP, against 50.55 / 48.90 spec-off -- +58.7% / +109.5%. Prefill 1740.7 @10K / 1383.1 @90K. KV pool 360,264 tok / 1.37x at 262K, peak free 778/798 MiB per card. WHY A SEPARATE ENGINE CHANNEL: MTP here is enabled on EVIDENCE, and that evidence is v0.27.1-specific. vllm#50021 was re-audited against the RUNNING image -- on v0.27.1 the fla package is NOT installed, fused_sigmoid_gating.py (the file docs/UPSTREAM.md cites as verified live in our pin) does NOT exist, the kernels moved into mamba/gdn/qwen_gdn_linear_attn.py, and mamba_ssm.py already clamps the negative case with tl.maximum(num_accepted - 1, 0). Then soaked: 120 agent-shaped multi-turn requests at ~7.4K prompt tok/turn, 120 ok / 0 fail, RestartCount=0, ZERO unspecified-launch-failure, ZERO illegal-address, ZERO Xid -- where the upstream repro faults at 7-10. LIMIT: 120 requests at ~7.4K ctx over 5 min, not a 262K-depth or hour-long soak, and the upper-bound case in the Qwen GDN callee is unaudited: strong evidence, not proof, and it does NOT clear the v0.25.1 pin the sibling slugs run on. First-party AMPERE datapoint (all prior #50021 reports are 5090/4090). RUNS util 0.95 via mem_util_unclamped, NOT 0.98: at 0.98 with MTP it OOMs at the prefill-90k segment -- a plain torch.OutOfMemoryError, no Xid, exit 0, i.e. MTP needs ~1 GB of its own headroom, not a GDN fault. max_num_seqs=2 is deliberate for a 262K-configured workload: the POOL is the binding constraint (~1.37 full-depth sessions), so admitting more does not add throughput, it just guarantees recompute-preemption at ~190 s per resume. A seqs=8 arm measured 354,406 tok / 1.35x / 756-776 MiB and is the right setting ONLY for short (~30K) agent turns. Chat template patched (see qwen38-reasoning-effort-template) -- the stock one 500s on reasoning_effort=high. NOT production: no verify-stress ladder, no soak-continuous, no quality pack, and every bench flagged the serving process paged out (~600 MB on a 29.7 GiB host at vm.swappiness=60), so nothing here is BENCHMARKS-grade.",
    ),
