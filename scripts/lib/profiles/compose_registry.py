...
COMPOSE_REGISTRY = {
    ...
    "llamacpp/qwen38-27b-single-iq4xs": _entry(
        model="qwen3.8-27b", weights_variant="unsloth-iq4xs", workload="long-ctx-single",
        engine="llama-cpp-local", drafter="qwen-mtp-builtin", kv_format="q8_0",
        tp=1, max_ctx=131072, max_num_seqs=1, mem_util=None,
        compose_path="models/qwen3.8-27b/llama-cpp/compose/single/unsloth-iq4xs/q8kv.yml",
        default_port=8086,
        kvcalc_key="SKIP",
        status="incubating",
        status_note="Qwen3.8-27B (Unsloth Dynamic UD-IQ4_XS GGUF, 14.3 GB) on a single 3090, mainline llama.cpp (llama-cpp-local pin, server-cuda-b10236). q8_0/q8_0 KV @131072, -b 4096 -ub 512, built-in MTP n=2 (--spec-type draft-mtp), text-only. ✅ FIRST BOOT 2026-08-14 (IQ4_NL artifact): launches via switch.sh --force, serves coherently, verify-full EXIT=0. ⭐ The embedded MTP head ENGAGES — 'creating MTP draft context against the target model', draft acceptance 0.68919 (51/74), mean len 2.38 at n=2 — so the b10236 pin loads this qwen35 GGUF AND drives its nextn head; engine compatibility is OBSERVED for that artifact. ⚠️ MEASURED VRAM 20,322 / 23,850 MiB (IQ4_NL artifact): the 32/32 layer split is NOT even — the cards sit 3.5 GB apart and card 1 is at 97% of 24,576 MiB. The even-split assumption below is therefore DISPROVEN, and the thin margin bears directly on fill depth. ⚠️ RE-QUANT 2026-08-19: unsloth removed IQ4_NL from the repo, so this slug now serves UD-IQ4_XS (13.27 GiB, ~1.95 GiB lighter). ✅ NEW ARTIFACT BOOTED 2026-08-19 on a 5090 (32 GB): launches via switch.sh --force, serves coherently, verify-full PASS + verify-stress PASS (boundary 5/5, recall ladder 3/3); the ceiling ladder FILLED to 120,320 tok (91% of n_ctx=131072) with VRAM free flat at 12,207 MB (Δ 0 across the ladder) — the 131K config is fillable, not just allocated, on this quant. /v1/models confirms the landed file: size 14,241,849,344 B, ftype IQ4_XS 4.25 bpw, n_ctx 131072, n_ctx_train 262144. STILL UNVALIDATED: no bench (NO TPS number is claimed), no soak, no 8-pack. MTP engagement CONFIRMED on this artifact 2026-08-19: container log shows 'creating MTP draft context against the target model' against the UD-IQ4_XS file, drafter ON, draft acceptance 0.66–0.92 on the large-sample tasks (mean len ~2.3–2.8). The 131072 ceiling was MEASURED on the IQ4_NL artifact; with the lighter weights the budget is: per docs/KV_MATH.md the 16 growing layers cost 16x4x256x2 = 32,768 elements/token, which at q8_0 (1.0625 B/elem) is 34,816 B/token = 4.25 GiB at 131K; against 23.40 GiB usable that leaves 13.27 (weights — landed 2026-08-19, /v1/models size 14,241,849,344 B; HF API figure was 14,252,845,984 bytes) + 4.25 (KV) + ~1.60 (buffers, back-solved from the qwen3.6-27b single-card config at -ub 512, identical growing geometry) = 19.12 GiB used, ~4.28 GiB headroom. ⚠️ UNIT TRAP: the HF page's '14.3 GB' is DECIMAL — 13.27 GiB in VRAM terms; weights_variant size_gb keeps the decimal 14.3 because the setup.sh disk gate wants the larger number, while this budget uses the GiB one. Solving the same budget for ctx with the lighter weights gives ~263,000 tokens — nominally past the 262,144 architectural max, NOT a green light (the ctx-scaling compute buffers + rs cache that refuted 196,608/262,144 on IQ4_NL are not in this budget) — and we deliberately ship the lower 131072 anyway: the buffer term is borrowed from a sibling model and this family has a documented alloc-vs-fill gap (qwen3.6-27b allocates 262144 and walls ~125K on the FA scratch), so an allocation that boots is not a fill depth. 163840+ is the obvious first step up once a NIAH ladder proves fill depth. ✅ RESOLVED 2026-08-19: this file loads on the b10236 pin — it booted and served on a 5090 (the earlier same-repo inference is now an observation). q8_0 KV rather than q4_0 is deliberate (stack serving-KV floor); q4_0 would double the ctx and is available via KV_TYPE as a speed exhibit, not a default. Vision is OFF because the mmproj projectors in the same HF repo were not downloaded — enabling it needs a third pull plus a kind:mmproj weights entry and a -vision.yml sibling. Promote to 🧪 on a clean boot + verify-full; to ⚠️/✅ only after the full gate.",
    ),
