...
#
#   With prefix caching enabled, TTFT for turn N should be proportional to
#   ONLY the new tokens in that turn (user msg + tool result). Without it,
#   TTFT grows proportionally to the full accumulated context.
#
#   A "good" result: TTFT stays roughly flat after session warmup (~300ms),
#   even as prompt_tokens approaches 33K (default 12-turn session). A "bad"
#   result: TTFT grows linearly with context (prefix caching not working).
#
# Fixture:
...
#   repo (filesystem paths redacted). Sizes range from 300 chars (ls output)
#   to 35K chars (large file reads), reaching ~53K accumulated prompt tokens
#   by turn 15. This matches real coding-agent workload patterns.
#
# Cliff 2 ceiling (vLLM only — llama.cpp is not affected):
...
#   sequence on every turn. On vLLM single-card 24 GB, TTFT degrades
#   noticeably above ~35K accumulated tokens (TTFT > 4 min) and requests
#   time out at ~74K. Default TURNS=12 caps at ~33K, safely below.
#   Use TURNS=14 only if investigating the cliff boundary on vLLM.
#
# Output:
#   Per-turn table (turn, prompt_tokens, ttft_ms, decode_tps)
#   TTFT growth analysis: flat = prefix caching working; linear = not
#
# Usage:
...
#   TURNS        Turns per session (1-15). Default: 12 (safe below Cliff 2)
...
for turn_idx in range(TURNS):
    ...
    if turn1_ttft and turn_idx > 0 and mean_ttft > 0:
        ...
        if ratio > 4.0:
            note = f"⚠  TTFT {ratio:.1f}× turn-1 (linear growth — cache miss?)"
        elif ratio > 2.0:
...
if len(filled) >= 2:
    ...
    print(f"\n{'─'*72}")
    print(f"  Prefix cache effectiveness ({active_turns} turns, {SESSIONS} sessions):")
    ...
    if ttft_growth <= 1.5:
        print("    ✓  TTFT stable — prefix caching is highly effective.")
    elif ttft_growth <= 2.5:
        print(f"    ~  TTFT grew modestly — partial cache benefit ({ttft_growth:.1f}× vs {token_growth:.1f}× context).")
    elif ttft_growth <= token_growth * 0.5:
        print(f"    ~  TTFT sub-linear — some cache benefit, could be better.")
    else:
        print(f"    ⚠  TTFT grew near-linearly — check --enable-prefix-caching and cache eviction.")
    print(f"    (Worst-case uncached: TTFT would grow {token_growth:.1f}× with context)")
    print(f"")
    print(f"  Note — DeltaNet SSM state is NOT prefix-cacheable (vLLM only). The above TTFT")
    print(f"  reflects attention KV caching only. SSM state recomputation scales")
    print(f"  O(n) with sequence length; on vLLM, TTFT degrades above ~35K tokens")
    print(f"  (TTFT > 4 min) and requests time out at ~74K (Cliff 2). Default TURNS=12")
    print(f"  stays safely below this threshold (~33K). llama.cpp is not affected.")
