...
assert_contains "$out" "VLLM_IMAGE=vllm/vllm-openai:v0.26.0"
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  assert_contains "$out" "image: vllm/vllm-openai:v0.26.0"
...
assert_contains "$out" "vllm/vllm-openai:v0.26.0"   # clean (vllm/dual → vllm-stable) bumped to v0.25.1
