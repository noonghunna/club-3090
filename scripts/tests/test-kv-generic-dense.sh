...
#
# Asserts:
#   1. tools/kv-calc.py --calibration still reports the exact line "Overall: 17/17 (100%)"
...
if ! grep -qxF "Overall: 17/17 (100%)" <<<"$CALIB_OUT"; then
  echo "FAIL: --calibration did not report the exact line 'Overall: 17/17 (100%)'" >&2
  echo "----- last 6 lines of --calibration output -----" >&2
  tail -6 <<<"$CALIB_OUT" >&2
  exit 1
fi
echo "PASS: --calibration reports 'Overall: 17/17 (100%)' (curated branches unchanged)"
