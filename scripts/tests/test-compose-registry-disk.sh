...
check(len(COMPOSE_REGISTRY) == 66, f"registry has 66 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 67, f"disk has 67 compose files (got {len(disk_paths)})")
