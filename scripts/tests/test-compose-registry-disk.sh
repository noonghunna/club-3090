...
check(len(COMPOSE_REGISTRY) == 135, f"registry expects 135 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 138, f"disk expects 138 compose files (got {len(disk_paths)})")
