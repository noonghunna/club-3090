...
check(len(COMPOSE_REGISTRY) == 103, f"registry has 100 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 104, f"disk has 101 compose files (got {len(disk_paths)})")
