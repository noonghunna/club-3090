...
check(len(COMPOSE_REGISTRY) == 84, f"registry has 84 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 85, f"disk has 85 compose files (got {len(disk_paths)})")
