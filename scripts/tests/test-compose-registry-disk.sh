...
check(len(COMPOSE_REGISTRY) == 91, f"registry has 91 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 92, f"disk has 92 compose files (got {len(disk_paths)})")
