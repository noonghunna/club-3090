...
check(len(COMPOSE_REGISTRY) == 101, f"registry has 101 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 102, f"disk has 102 compose files (got {len(disk_paths)})")
