...
check(len(COMPOSE_REGISTRY) == 68, f"registry has 68 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 69, f"disk has 69 compose files (got {len(disk_paths)})")
