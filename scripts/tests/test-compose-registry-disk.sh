...
check(len(COMPOSE_REGISTRY) == 106, f"registry expects 106 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 107, f"disk expects 107 compose files (got {len(disk_paths)})")
