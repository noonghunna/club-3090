...
check(len(COMPOSE_REGISTRY) == 134, f"registry expects 134 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 137, f"disk expects 137 compose files (got {len(disk_paths)})")
