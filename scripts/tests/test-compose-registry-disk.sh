...
check(len(COMPOSE_REGISTRY) == 83, f"registry has 83 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 84, f"disk has 84 compose files (got {len(disk_paths)})")
