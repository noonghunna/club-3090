...
check(len(COMPOSE_REGISTRY) == 86, f"registry has 86 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 87, f"disk has 87 compose files (got {len(disk_paths)})")
