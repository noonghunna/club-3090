        ...
        for layer_name in kv_cache_group_spec.layer_names:
            ...
            if isinstance(kv_cache_spec, AttentionSpec):
                ...
                if kv_cache_spec.page_size_padded is not None:
                    # PR #40391: split the page_size_padded handling by backend
                    # type. MLA backends keep the strided-view path (kv_cache_shape[0]
                    # == num_blocks holds for them); standard attention backends
                    # whose shape starts with K/V dim of size 2 need a different
                    # path that pre-pads the per-block storage and uses a
                    # contiguous view. This unblocks Gemma 4 + per-token-head fp8 KV
                    # on hybrid attention models with mixed head_dims (sliding=256
                    # vs global=512), which is upstream-blocked at
                    # kv_cache_utils.py:unify_kv_cache_spec_page_size without
                    # this branching.
                    if attn_backend.is_mla():
                        # MLA: strided view (existing behavior — assumes
                        ...
                        # Standard attention: compute padded shape via the new
                        # helper and use a contiguous view. This is what
                        # PR #40391 adds to make Gemma 4 (head_dim=256 sliding
                        # vs head_dim=512 global) work with per-token-head fp8 KV.
