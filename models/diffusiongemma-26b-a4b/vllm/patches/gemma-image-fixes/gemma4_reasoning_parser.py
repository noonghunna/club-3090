...
class Gemma4ReasoningParser(BaseThinkingReasoningParser):
    ...
    def extract_reasoning_streaming(
        ...
        # ---- club-3090 fix: strip a leaked <|channel> start token ----
        # The base streaming parser (BaseThinkingReasoningParser) only removes
        # the start token when it can pair it with the end token in the SAME
        # delta (its "start in delta, end in delta" branch uses .find()).
        # DiffusionGemma is a block-diffusion LM: it decodes a whole ~256-token
        # canvas per SSE delta, so "<|channel>thought\n...reasoning..." arrives
        # in ONE delta while the closing "<channel|>" lands in a LATER canvas.
        # That hits the base parser's "start token in delta, no end token"
        # branch, which returns delta_text verbatim — start token included —
        # leaking the literal "<|channel>thought\n" into the reasoning stream
        # (the offline / non-streaming path strips it correctly). Strip the
        # leading start token here, before the thought\n label logic below, so
        # the existing _THOUGHT_PREFIX handling can then strip "thought\n".
        # The start token decodes atomically from one token id (skip_special
        # _tokens=False), so it is never split across deltas; a startswith
        # check is sufficient and never matches genuine reasoning content.
        if result.reasoning.startswith(self.start_token):
            result.reasoning = result.reasoning[len(self.start_token) :]
            if not result.reasoning:
                # Delta was nothing but the start token — emit nothing yet.
                return None
        # ---- end club-3090 fix ----
