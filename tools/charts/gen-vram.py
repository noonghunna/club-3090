...
rows_single = [
    ...
    ("vLLM single 198K + vision\nTQ3 KV, mem-util 0.98 (post-PN12)",
        [(7.0, "weights", C_W), (4.5, "KV (TQ3)", C_KV), (0.5, "vision", C_VIS),
         (10.3, "activations + workspace", C_ACT), (1.7, "free", C_FREE)]),
    ("vLLM single 218K text-only\nTQ3 KV, mem-util 0.985 (post-PN12)",
        [(7.0, "weights", C_W), (5.0, "KV (TQ3)", C_KV),
         (10.5, "activations + workspace", C_ACT), (1.5, "free", C_FREE)]),
...
rows_dual = [
    ...
    ("dual-turbo\n262K · 4 streams · TQ3 KV",
        [(7.0, "weights", C_W), (6.0, "KV (TQ3)", C_KV), (0.5, "vision", C_VIS),
         (10.6, "activations (4 streams)", C_ACT), (-0.1, "free", C_FREE)]),
...
draw_panel(ax_bot, rows_dual,   "Dual 3090 — vLLM (TP=2), per-card breakdown (each card holds half the weights + half the KV) — all 4 configs run vLLM, dual-dflash uses vLLM's DFlash spec-decode")
...
draw_panel(ax, rows_dual, "Dual 3090 — vLLM (TP=2), per-card breakdown (each card holds half the weights + half the KV) — all 4 configs run vLLM, dual-dflash uses vLLM's DFlash spec-decode")
