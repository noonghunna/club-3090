...
| llama.cpp PR [#22673](https://github.com/ggml-org/llama.cpp/pull/22673) MTP, Docker build (`llama-cpp-mtp:latest` from `am17an/llama.cpp:mtp-clean`, `havenoammo/Qwen3.6-27B-MTP-UD-Q3_K_XL.gguf` + `--spec-type mtp --spec-draft-n-max 3` + q4_0 KV) | @eriklabianca (1× RTX 5090 Laptop 24 GB, Lenovo Legion 7i Pro, WSL2, **~166W TGP, Performance mode**) | UD-Q3_K_XL MTP (heads embedded in main GGUF) | 262K | **49.54 / 65.75** | ~23.1 GB | 2026-05-09 | **First club-3090 Docker-based llama.cpp MTP data point.** Decode 49.54 / 65.75 TPS, TTFT 232 / 228 ms. CV 1.2%/2.0%. **+71% narr / +127% code vs same-rig no-MTP baseline** (~29 TPS). MTP heads embedded in main GGUF — no separate `--model-draft` needed. Custom image `models/qwen3.6-27b/llama-cpp/build/Dockerfile.mtp`; compose at `compose/mtp/docker-compose.yml`. **Performance mode TGP (~166W)** vs 3090-row baselines at 350W cap — re-bench at higher TGP pending. ⚠ Agentic stress shows TPS collapse at turn 7 (~9K accumulated ctx, 10.5 TPS vs 71–76 TPS at turns 1–6) — likely am17an fork behaviour with spec-decode at longer contexts; canonical short-prompt bench unaffected. Dockerfile fixes: CUDA VMM stub symlink + `-DGGML_CUDA_NO_VMM=ON`, staging-dir SO copy, `libgomp1`. |
