...
Endpoints:
  POST /produce  {brief, continuity?, shots?, backend?}  -> {job_id}
  GET  /produce/health                                   -> {ok, active}
  GET  /job/<job_id>  -> {status: planning|rendering|done|error, phase, frac, title,
                          final, gallery_url, error}
...
def _run_job(job_id: str, brief: str, continuity: str, shots: int, backend: str) -> None:
    ...
    try:
        ...
        plan, arts = planner.plan_from_brief(
            brief, registry.load(), n_shots=shots, continuity=continuity,
        ...
        summary = run_production(
            ...
            extra_artifacts=arts, progress_cb=lambda m, f: _set(job_id, phase=m, frac=f),
        )
        final = summary["final"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/produce/health":
            return self._json(200, {"ok": True, "director": config.DIRECTOR_MODEL, "active": _active()})
    ...
    def do_POST(self):
        ...
        threading.Thread(
            target=_run_job,
            args=(job_id, brief, b.get("continuity", "storyboard"), int(b.get("shots", 3)),
                  b.get("backend", "live")),
            daemon=True,
        ).start()
